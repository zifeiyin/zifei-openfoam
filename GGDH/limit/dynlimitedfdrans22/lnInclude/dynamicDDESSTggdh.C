...
//	dimensionedScalar("lzero", dimLength*dimLength, 0.0)
    ...
    mesh_,
    dimensionedScalar("crans", dimLength/dimLength, -1.8)
    ),
    ctdyn_
    (
        IOobject
...
void dynamicDDESSTggdh::update_nut()
{
    ...
    //volVectorField Mi_( filter_((tauij_&Tgradf)/omega_) - scalar(4)*(filter_(tauij_/omega_)&filter_(Tgradf)) );

    //volVectorField N1_ = sqr(CDES_*delta_)*(scalar(4)*filter_(omega_*S) - filter_(omega_)*filter_(S))&Fi_;
    ...
    crans_ = scalar(-1.0) + fdl_*(scalar(-2.2) - scalar(-1.0));

    cprt_ = crans_ + fd_ * (ctdyn_ - crans_);
...
//--------------k/omega instead of nut for rd---------------------------
    ...
    fdl_ = scalar(1) - tanh(pow(scalar(1.5)*Cd1_*rdl_, Cd2_*scalar(2.0)));
