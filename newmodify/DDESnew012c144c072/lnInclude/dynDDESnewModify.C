...
void dynDDESnewModify::update_nut()
{
    ...
    volScalarField Prtsgs_ = max(fvc::average(NUM_)/fvc::average(DENOM_),scalar(0.01));

    PrtRANS_ = scalar(1.44) * (scalar(1)-fdl_) + fdl_ * scalar(0.72);

    Prt_ = PrtRANS_ + fd_*(Prtsgs_ - PrtRANS_);
