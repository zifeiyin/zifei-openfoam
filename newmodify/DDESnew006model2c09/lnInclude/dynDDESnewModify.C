...
defineTypeNameAndDebug(dynDDESnewModify, 0);
addToRunTimeSelectionTable(RASModel, dynDDESnewModify, dictionary);
volScalarField dynDDESnewModify::cD
(
...
dynDDESnewModify::dynDDESnewModify
(
...
//	dimensionedScalar("lzero", dimLength*dimLength, 0.0)
    ...
    mesh_,
    dimensionedScalar("alphat", dimVelocity*dimLength, 1e-12)
    ),
    CFL_
    (
        IOobject
    ...
    mesh_,
    dimensionedScalar("CFL", dimVelocity/dimLength, 0.0)
    ),
    y_(mesh_),
...
tmp<volSymmTensorField> dynDDESnewModify::R() const
{
...
tmp<volSymmTensorField> dynDDESnewModify::devReff() const
{
    return tmp<volSymmTensorField>
    (
        new volSymmTensorField
...
tmp<fvVectorMatrix> dynDDESnewModify::divDevReff(volVectorField& U) const
{
    return
    (
...
tmp<fvVectorMatrix> dynDDESnewModify::divDevRhoReff
(
...
void dynDDESnewModify::update_nut()
{
    ...
    scalar deltaT_ =  U_.time().deltaT().value();

    volSymmTensorField Tij_(dev(filter_(sqr(U())) - (sqr(filter_(U())))));
    volVectorField F_( filter_(U()*Tfield ) - ( filter_(U())*filter_(Tfield) ) );
    volSymmTensorField S(symm(fvc::grad(U_)));
    volVectorField Tgrad_ = fvc::grad(Tfield);

    volSymmTensorField ModelTij_ 
    (
        scalar(4)*(-scalar(2)*filter_(nut_)*filter_(S)) - (-scalar(2)*filter_(nut_*S))
    );

    volVectorField ModelFi_
    (
        scalar(4)*filter_(nut_)*filter_(Tgrad_) - filter_(nut_*Tgrad_)
    );

    volScalarField NUM_ = (Tij_&ModelFi_) & (ModelTij_&F_);
    volScalarField DENOM_ = (ModelTij_&F_) & (ModelTij_&F_);

    DENOM_.max(VSMALL);

    volScalarField Prtsgs_ = max(fvc::average(NUM_)/fvc::average(DENOM_),scalar(0.001));

    //PrtRANS_ = scalar(1.44) * (scalar(1)-fdl_) + fdl_ * scalar(0.9);
    PrtRANS_ = scalar(0.9);

    volScalarField lRANSLES_ = (lDDES_ - lRANS_)/(lLES_ - lRANS_);

    Prt_ = PrtRANS_ + lRANSLES_*(Prtsgs_ - PrtRANS_);

    alphat_ = nut_/Prt_;

    CFL_ = deltaT_*scalar(2.0)*(nu()/0.7 + alphat_)*(scalar(1.0)/sqr(hmax_)+ scalar(1.0)/sqr(hmin_) + scalar(1.0)/sqr(pow(cube_root_vol_,3.0)/hmin_/hmax_)   );

    nut_.correctBoundaryConditions();
}

void dynDDESnewModify::correct()
{
    RASModel::correct();
...
//--------------k/omega instead of nut for rd---------------------------
    ...
    volScalarField G("dynDDESnewModify:G", nut_*2*magSqr(symm(fvc::grad(U_))));
...
//        alpha_*G*omega_/k_
    ...
    Info << "min Prt = " << min(Prt_)<< "  max prt = " << max(Prt_)<< endl;
    Info << "max CFL = " << max(CFL_) << endl;
}
void dynDDESnewModify::calcdelta()
{
	label nD = mesh_.nGeometricD();
    if(nD==2)
    {
        WarningIn("dynDDESnewModify::calcDelta()")
