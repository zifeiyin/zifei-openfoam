...
if (t_  > startT + 2.0*deltaT_)
 ...
 //Info << "counter mod 6" << counter%6 << endl;
        if (counter%3 == 0)
        {   
        	  ...
        	  forAll(Uinf,faceI)
                {
                    ...
                    upert[i][j] = upert[i][j] * maxPerturbation_/xMaxPert;
                    vpert[i][j] = vpert[i][j] * maxPerturbation_/yMaxPert;
                    wpert[i][j] = wpert[i][j] * maxPerturbation_/zMaxPert;
                }
